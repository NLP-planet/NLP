'''
@Project ：NLP 
@File    ：基于sentenceBert双塔语义匹配模型训练及预测.py
@IDE     ：PyCharm 
@Author  ：Gaogz
@Date    ：2024/5/8 01:02 
@Desc    ：
...
max_columns(int): 一行中排列几张图，默认为一行2张（2
个变量）的图。
"""
if params is None:
    params = []
...
"""
Sentence
Transomer实现, 双塔网络, 精度适中, 计算速度快。
Paper
Reference: https: // arxiv.org / pdf / 1908.10084.pdf
Code
Reference: https: // github.com / PaddlePaddle / PaddleNLP / blob / develop / examples / text_matching / sentence_transformers / model.py
...
Args:
encoder(transformers.PretrainedModel): backbone, 默认使用
ernie
3.0
...
"""
forward
函数，输入query句子和doc_embedding向量，将query句子过一遍模型得到
query
...
"""
输入句子，返回这个句子的embedding，用于事先计算doc
embedding并存储。
...
"""
forward
函数，输入query和doc的向量，返回两个向量相似 / 不相似的二维向量。
